...
export class Article {
  _id: number
  _name: string
  _isDeleted: Ref<boolean>
  _author: string
  _timeLastModified: Date
  _category: string
  _brief?: string
  _content?: string
  _tags?: Set<string>
  ...
  constructor(
    ...
    if (id < 0 || !Number.isInteger(id)) throw new Error('_id must be positive integer')
    this._id = id
    this._name = name
    this._isDeleted = ref(isDeleted)
    this._author = author
  ...
  public get TagsArray() {
    if (typeof this._tags == 'undefined') return []
    else return [...this._tags]
  }

  public set DeleteStatus(status: boolean) {
    this._isDeleted.value = status
  }
}
...
function getArticleByQuery(_name: string, _id?: number) {
  for (const item of ArticleList) {
    if (typeof item._content == 'undefined') return null
    if (item._name === _name || item._id === _id) {
      return item
    }
  }
  return null
...
function getAllTags() {
  ...
  for (const item of ArticleList) {
    if (typeof item._tags != 'undefined') {
      tagSet = union(tagSet, item._tags)
    }
  }
  return tagSet
}

export { getArticleList, getArticleByQuery, getAllTags }
